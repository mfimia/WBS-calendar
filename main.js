...
const createEvent = (event, text, day, month, year) => {
    ...
    addInEventListSidebar(startTime, endTime, text, `${day}.${month}.${year}`);
};

// To add events in side bar
const addInEventListSidebar = (startTime, endTime, text, date) => {
    document.querySelector("#event-lists").innerHTML += `<div class="event-item"><p class="event-item-content">${date} ${startTime} - ${endTime} <br>${text}</p></div>`;

};
if (EVENTS) {
    console.log(EVENTS);
    EVENTS.forEach(item => {
        addInEventListSidebar(item.startTime, item.endTime, item.title, `${item.numWeekday}.${item.numMonth}.${item.year}`);
    });
}
